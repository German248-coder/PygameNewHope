...
sound_tie_shot = pygame.mixer.Sound('sounds/звук стрельбы сид истребителя.ogg')
sound_xwing_shot = pygame.mixer.Sound('sounds/звук выстрела x-wing.ogg')
sound_tie_fly  = pygame.mixer.Sound('sounds/Звук полёта имперского истребителя.ogg')
sound_xwing_fly = pygame.mixer.Sound('sounds/звук полёта x-wing.ogg')
sound_tie_bum = pygame.mixer.Sound('sounds/взрыв машины.ogg')

# Запускаем звук полёта X-Wing по кругу
sound_xwing_fly.play(-1)
...
def enemy_shoot():
    ...
    for enemy in enemies:
        if enemy[1] > 0 and random.random() < 0.015: # 1.5% шанс выстрела
            ...
            sound_tie_shot.play() # << ВАЖНО: звук залпа врага
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1: # Левая кнопка мыши
                ...
                sound_xwing_shot.play() # Звук выстрела X-Wing
    ...
    for enemy in enemies:
        ...
        sound_tie_fly.play(-1) # Звук полёта сил истребителей
    ...
    for explosion in explosions[:]: # Делаем копию списка для безопасного удаления
        ...
        if explosion[2] > 30: # Удаляем старые взрывы
            ...
            sound_tie_fly.stop() # Прерывание полёта после уничтожения сида истребителя
            sound_tie_bum.play() # Взрыв вражеского истребителя
